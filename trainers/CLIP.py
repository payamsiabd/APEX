...
class CustomCLIP(nn.Module):
    def __init__(self, cfg, classnames, clip_model):
        ...
        # dir_path = "/projects/academic/alipour/payamabd/FedMMA3/Figs"
        # topk = 500
        # channel_std = torch.load(f"{dir_path}/hist.pt", map_location="cpu")

        # sorted_std, sorted_idx = torch.sort(channel_std, descending=False)
        # self.top_std_indices = sorted_idx[:topk]
    ...
    def forward(self, image, idx = None):
        ...
        if not self.training:
            with torch.no_grad():
                self.all_features.append(image_features.cpu())
                self.all_features_text.append(text_features.cpu())

        # image_features = image_features[:,self.top_std_indices]
        # text_features = text_features[:,self.top_std_indices]
