...
class ConcatLinearFuse(nn.Module):
    """y = W [x1 || x2] + b"""
    def __init__(self, dim: int):
        super().__init__()
        self.projection = nn.Linear(2*dim, dim)

    def forward(self, x1, cls_token):
        R = x1.size(0)
        cls_tiled = cls_token.expand(R,-1,-1)
        return self.projection(torch.cat([x1, cls_tiled], dim=-1)) 
...
class ResidualAttentionBlock(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor):
        ...
        if self.model == "CLIP":
            trans_layers = inputs[1]
            x = inputs[0]
            x = x + self.attention(self.ln_1(x))
            x = x + self.mlp(self.ln_2(x))
            trans_layers[self.layer]=x
            return [x, trans_layers]
            ...
            return [x, compound_rep_tokens, counter, trans_layers] # return again as a list, so that nn.seq can work   
...
class Transformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        return self.resblocks(x +[self.trans_layers])
